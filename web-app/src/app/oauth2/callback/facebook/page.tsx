...
function CallbackFacebookContent() {
    ...
    const handleSendCode = useCallback(async (code: string) => {
        ...
        try {
            ...
            if (data.result && data.result.accessToken) {
                ...
                setError('Không thể xử lý phản hồi từ Facebook. Vui lòng thử lại sau.');
            ...
            if (errorCode === '401') {
                ...
                setError('Tài khoản Facebook của bạn không có quyền truy cập ứng dụng này.');
                setErrorCode('403');
            } else if (errorCode === '500') {
                setError('Máy chủ đang bảo trì. Vui lòng thử lại sau vài phút.');
                setErrorCode('500');
            } else {
                setError(`Lỗi xác thực Facebook: ${errorMessage}`);
    ...
    return (
        <AuthCallbackLayout
            ...
            description="Đang xử lý thông tin xác thực từ Facebook"
        >
            {isLoading ? (
                <AuthLoading
                    message="Đang xác thực với Facebook"
                    provider="facebook"
                />
            ) : error ? (
                <AuthError
    ...
    return (
        <Suspense fallback={
            <AuthCallbackLayout title="Đang tải..." description="Đang khởi tạo xác thực">
                <AuthLoading message="Đang tải" provider="facebook" />
            </AuthCallbackLayout>
        }>
